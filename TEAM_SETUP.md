...
- 개인 화면 설정: 검색/정렬/숨김/빠른 필터/대시보드 포커스는 보드별 `localStorage`에 저장된다.
- 담당/마감: 카드별 담당자와 마감일은 카드 데이터에 함께 저장되고 CSV에도 포함된다.
- 활동 기록: 변경 유형, 카드명, 상세 변경 내용, 작성자, 시간이 함께 저장된다.
- 보드 정리: 기한 초과, 담당 없음, 오래된 미완료, 빈 섹션을 한 번에 점검한다.
...
2. `오늘 볼 것`에서 막힘, 중요, 최근 결정, 첨부, 기한 초과, 기한 임박, 오래된 미완료를 확인한다.
3. `중요`, `막힘`, `결정`, `첨부 있음` 빠른 필터로 지금 볼 카드만 좁힌다.
4. 활동 기록에서 카드명으로 최근 변경을 확인한다.
5. `보드 정리`에서 담당 없음, 기한 초과, 빈 섹션을 정리한다.
6. 중요한 작업을 정리한 뒤 `백업`을 눌러 파일로 보관한다.
7. 다른 컴퓨터나 브라우저에서 쓰려면 `복원`으로 같은 데이터를 불러온다.
8. CSV(Comma-Separated Values, 콤마 구분 값)는 작업 목록을 엑셀에서 볼 때 사용한다.
