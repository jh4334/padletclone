...
- 개인 화면 설정: 검색/정렬/숨김/빠른 필터는 보드별 `localStorage`에 저장된다.
...
2. `중요`, `막힘`, `결정`, `첨부 있음` 빠른 필터로 지금 볼 카드만 좁힌다.
3. 중요한 작업을 정리한 뒤 `백업`을 눌러 파일로 보관한다.
4. 다른 컴퓨터나 브라우저에서 쓰려면 `복원`으로 같은 데이터를 불러온다.
5. CSV(Comma-Separated Values, 콤마 구분 값)는 작업 목록을 엑셀에서 볼 때 사용한다.
